            ...
            CREATIVE_MODE_TAB.register("element_item_tab", () -> CreativeModeTab.builder()
                    ...
                    .displayItems((parameters, output) -> {
                        ...
                        for (String[] element : ElementItem.ELEMENTS) {
                            ...
                            // 添加该元素的各种物品形式
                            output.accept(ElementItem.getIngot(elementName)); // 锭
                            output.accept(ElementItem.getNugget(elementName)); // 粒
                            output.accept(ElementItem.getDust(elementName)); // 粉
                            output.accept(ElementItem.getPlate(elementName)); // 板
                            output.accept(ElementItem.getRod(elementName)); // 棍
                            output.accept(ElementItem.getWire(elementName)); // 线
                            output.accept(ElementItem.getGear(elementName)); // 齿轮
                            output.accept(ElementItem.getRaw(elementName)); // 粗矿
                            output.accept(ElementItem.getCrushedRaw(elementName)); // 粉碎粗矿
                        }
                        ...
                        for (String[] cylinder : ElementItem.GAS_ELEMENTS) {
                            ...
                            // 添加该元素的各种形式
                            output.accept(ElementItem.getCylinder(cylinderName)); // 元素气体瓶
                        }

                        output.accept(ElementItem.GAS_CYLINDER); // 气瓶
                    })
                    .build());
